...
#         st.caption("Enter what item you wish to produce (ex. red_science).")
    ...
    # delete item button
    with col3:
        st.button("Click to remove items", on_click=delete_target(i))
        
    st.session_state.targets[i]['item'] = st.session_state[f"item_{i}"]
    st.session_state.targets[i]['rate'] = st.session_state[f"rate_{i}"]
    
    
# call requirements and display results
